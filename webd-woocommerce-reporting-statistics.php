...
include_once( plugin_dir_path(__FILE__) ."/class-admin.php"); 
 ...
 class webdWoocommerceReportingStatistics extends webdWoocommerceReportingStatisticsAdmin{
		...
		public function BackEndScripts(){
			...
			wp_enqueue_style( "webd-woocommerce-reporting-statistics"."adminCss", plugins_url( "/css/backend.css?v=gssσvbs", __FILE__ ) );	
			...
			wp_enqueue_script( "webd-woocommerce-reporting-statistics"."adminJs", plugins_url( "/js/backend.js?v=sbgsσssv", __FILE__ ) , array('jquery','jquery-ui-accordion','jquery-ui-tabs','jquery-ui-datepicker') , null, true);	
						
			wp_enqueue_style( "webd-woocommerce-reporting-statistics"."_fa", plugins_url( '/css/font-awesome.min.css', __FILE__ ));

			$page = $this->slug;
			//$tab = array( 'all', 'years' ,'months' );
			$tab = array( 'all' );
